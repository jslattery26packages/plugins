...
- (void)torchOnWithResult:(FlutterResult)result API_AVAILABLE(ios(10))  {
	if (!_captureDevice.hasTorch) {
		result([FlutterError errorWithCode:@"torchOnFailed" message:@"Device does not support torch mode" details:nil]);
		return;
	}
	if (!_captureDevice.isTorchAvailable) {
		result([FlutterError errorWithCode:@"torchOnFailed" message:@"Camera does not support torch mode" details:nil]);
		return;
	}
...
- (void)torchOffWithResult:(FlutterResult)result API_AVAILABLE(ios(10))  {
	if (!_captureDevice.hasTorch) {
		result([FlutterError errorWithCode:@"torchOffFailed" message:@"Device does not support torch mode" details:nil]);
		return;
	}
	if (!_captureDevice.isTorchAvailable) {
		result([FlutterError errorWithCode:@"torchOffFailed" message:@"Camera does not support torch mode" details:nil]);
		return;
	}
...
- (void)handleMethodCallAsync:(FlutterMethodCall *)call result:(FlutterResult)result {
	if ([@"availableCameras" isEqualToString:call.method]) {
		...
		[_camera torchOnWithResult:result];
	} else if ([@"torchOff" isEqualToString: call.method]) {
		[_camera torchOffWithResult:result];
